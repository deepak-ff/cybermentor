...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--host", default="127.0.0.1", help="target host to port-scan (default 127.0.0.1)")
    p.add_argument("--ports", default="1-1024", help="port range or list, e.g. '80' or '1-1024' (default 1-1024)")
    p.add_argument("--timeout", type=float, default=1.0, help="socket connect timeout seconds (default 1.0)")
    p.add_argument("--threads", type=int, default=256, help="scanning threads (default 256)")
    p.add_argument("--skip-scan", action="store_true", help="skip the TCP port scan")
    p.add_argument("--show-speedup", action="store_true", help="compare concurrent vs sequential scanning")
    p.add_argument("--out", default="reports", help="output directory for reports (default 'reports')")
    p.add_argument("--hostname", default="localhost", help="label used in the report (default localhost)")
    return p
...
def main(argv=None) -> int:
    ...
    print(f"[*] Running configuration checks on {hostname} ...")
    checks = run_all_checks(hostname)

    open_ports = []
    speedup_ms = None
    if not args.skip_scan:
        print(f"[*] Scanning {args.host}:{args.ports} (threads={args.threads}) ...")
        open_ports, elapsed_ms = timed_scan(args.host, ports, timeout=args.timeout, threads=args.threads)
        print(f"[*] Port scan complete in {elapsed_ms} ms; {len(open_ports)} open port(s).")
        if args.show_speedup:
            ...
            speedup_ms = seq_ms
            print(f"[*] Speed-up vs sequential: {seq_ms} ms -> {elapsed_ms} ms "
    ...
    print("[+] Summary:", ", ".join(f"{lvl.value}: {counts[lvl]}" for lvl in Level if counts[lvl]))
